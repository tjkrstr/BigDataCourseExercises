...
all_word = txtFiles.flatMap(lambda s: s[1].lower().split())
...
word_map = all_word.map(lambda s: (s, sentiment(s)))
...
word_map = word_map.reduceByKey(add)
#word_map = word_map.collect()
#word_map = word_map.reduce
#word_map = word_map.sortBy(lambda s: s[1], ascending=False)
print(word_map.collect())
print(word_map)

#total = word_map.reduce(lambda s: s[1])
#word_map.collect()
#print(total)
word_map = word_map.map(lambda s: s[1])
total = word_map.reduce(lambda s, t: s + t)
print(total)